...
public final class OSCChannel: NSObject {
	let udpServer = GCDAsyncUdpSocket()
	let udpDelegate = OSCChannelDelegate()
...
extension OSCChannel {
	...
	func dispatch(_ message: OSCMessage, timeTag: OSCTimeTag) {
		dispatchQueue.async {
			self.handlers.forEach({ handler in handler(message, timeTag) })
		}
	}
...
class OSCChannelDelegate: NSObject, GCDAsyncUdpSocketDelegate {
	weak var oscServer: OSCChannel?
	
	init(oscServer: OSCChannel? = nil) {
